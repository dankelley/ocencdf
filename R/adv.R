...
adv2ncdf <- function(x, varTable = NULL, ncfile = NULL, force_v4 = TRUE, debug = 0) {
    ...
    timeFast <- ncdim_def(
        name = "TIME_FAST", units = "", vals = seq_len(timeFastLen), create_dimvar = FALSE,
        longname = "seconds since 1970-01-01 UTC"
    )
    if (anySlow) {
        timeSlow <- ncdim_def(
            name = "TIME_SLOW", units = "", vals = seq_len(timeSlowLen), create_dimvar = FALSE,
            longname = "seconds since 1970-01-01 UTC"
        )
    }
    ...
    for (name in dataNames) {
        ...
        if (is.matrix(item)) {
            ...
            if (length(item) == timeSlowLen) {
                ...
                stop(
                    "item \"", name, "\" has length ", length(item), " but it should be either ", timeSlowLen,
                    " or ", timeFastLen
                )
            }
        }
