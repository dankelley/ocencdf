...
Loading this file verifies that the variable names are not in oce convention.
Also, notice that the flags are stored in variables with names ending in `_QC`,
which is the system used in the Argo netcdf format, rather than being stored in
a list within the metadata slot (which gets displayed separately for conventional
oce objects).
...
Proper oce names are recovered by passing the object through [as.ctd()], as follows.
